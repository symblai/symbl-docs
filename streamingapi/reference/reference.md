...
```disconnectOnStopRequest``` <br/> <font color="orange"> LABS </font> | Optional | `true` or `false` | This parameter allows you to set your Streaming API connection in such a way that even when the `stop_request` is sent, the connection does not drop-off and only the processing is stopped. <br/> <br/> This allows you to stop and start the Streaming API processing without dropping the WebSocket connection, so that you can stop and resume the processing in the middle of the session/call/meeting and optimize the Streaming API usage costs. <br/> <br/> When `disconnectOnStopRequest` is set to `false`, the WebSocket connection stays connected but the processing is stopped.  The default value is `true`. |
